...
const OrderInfo:React.FC<OrderInfoProps> = ({ order, fetchTravels }) => {
...
const handleClick = async () => {
  ...
  return (
    ...
    >
        <div className="mt-[2rem] pt-[2rem] border-t border-gray/25">
            <div className="flex xl:flex-row flex-col xl:items-start xl:justify-between items-center max-xl:gap-[2rem]">
                    ...
                    <div className="flex items-stretch justify-end lg:flex-row flex-col gap-[1.5rem]">
                        ...
                        <div className="flex items-stretch gap-[0.5rem]">
                            ...
                            <div className='flex flex-col justify-between max-lg:items-center flex-1 h-full'>
                                <div className='flex items-center font-open-sans font-[400]'>
                                    ...
                                    <p className='xl:text-[1rem] text-[1.333rem] text-dark-gray line-clamp-1 text-nowrap'>{ order.departure_place.label[getLocale(locale)] }</p>
                                </div>
                                <div className='flex items-center font-open-sans font-[400]'>
                                    <p className='xl:text-[1rem] text-[1.333rem] text-gray/75 mr-[0.5rem]'>{ route_t('finish') }:</p>
                                    <p className='xl:text-[1rem] text-[1.333rem] text-dark-gray line-clamp-1 text-nowrap'>{ order.arrival_place.label[getLocale(locale)] }</p>
                                </div>
                            </div>
                        </div>
                    </div>
                ...
                <div className="flex justify-end mt-[1.125rem]">
                    <div className='flex items-center gap-[0.25rem]' onClick={handleClick}>
                        <img src="/icons/route-card-icons/icon-delete.svg" alt="delete" className='lg:size-[1rem] size-[1.333rem]' />
                        <UnderlinedText text={t('delete-order')}/>
                    </div>
                </div>
            </div>
