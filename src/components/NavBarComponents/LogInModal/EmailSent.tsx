...
const EmailSent: React.FC<EmailSentProps> = ({ setIsOpen, setModalContent, sentEmail }) => {

  const t = useTranslations("EmailSentModal")
  ...
  return (
    <div className='py-[4rem] px-[3rem] relative'>
        ...
        <p className='text-dark-gray font-open-sans font-[400] text-[1.125rem] text-center mt-[1rem]'>{ t('message') } <span className='font-bold'>{ sentEmail }</span></p>
        ...
        <div className='flex items-center justify-center gap-[2rem] mt-[1.5rem]'>
            <div className='font-open-sans text-gray text-[1.125rem] font-[400] text-center flex flex-wrap justify-center'>
                <span onMouseEnter={() => setMouseHover1(true)} onMouseLeave={() => setMouseHover1(false)} className="relative">
                    <span onClick={() => setModalContent('ResetPass')} className='cursor-pointer'>{ t('send-again') }</span>
                    <motion.div 
